<script lang="ts">
    ...
    allTasksCache.subscribe((tasks: Task[]) => {
        if (filterParams.id) {
            const task = tasks.find((task: Task) => task.id === filterParams.id);
            displayTask =  task;
            return;
        }
        ...
        if (filterParams.completed) {
            ...
            tasks = tasks.filter((task: Task) => !task.complete || (task.scheduled_date && task.scheduled_date >= startOfDay(new Date())))
        }

        groupings = sortTasksByDate(tasks, reverse).reduce((accum: any, task: Task) => {
...
<div>
    <ul class="contains-task-list todoist-task-list">
        {#if filterParams.id}
            {#if displayTask}
                <TaskListItem task={displayTask} plugin={plugin} />
            {:else}
                <em style="font-size:12px;color:grey;text-align:center">Deleted task: {filterParams.id}</em>
            {/if}
        {:else}
            {#each Object.keys(groupings) as grouping, index}
                <DayView {plugin} dayLabel={grouping} tasks={groupings[grouping]}></DayView>
            {/each}
        {/if}
    </ul>
</div>
