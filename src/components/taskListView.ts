...
export class TaskListView extends ItemView {
    ...
    getDisplayText(): string {
        if (this.type === TASK_LIST_TYPES.today) return `Tasks - ${format(new Date(), 'EEEE, MMM d')}`;
        if (this.type === TASK_LIST_TYPES.completed) return `Completed Tasks`;
        if (this.type === TASK_LIST_TYPES.thisWeek) return `Tasks this Week (${format(startOfWeek(new Date(), {weekStartsOn:1}), 'MM/d')}-${format(endOfWeek(new Date(), {weekStartsOn:1}), 'MM/d')})`;
