...
export class SettingsTab extends PluginSettingTab {
  ...
  display(): void {
    ...
    new Setting(containerEl)
      ...
      .setDesc('This indicates the date that the Task Completion goal starts incrementing (by the number in Daily Bandwidth. Format as yyyy-MM-dd (i.e. 2023-02-01)')
      .addText((text) => {
        ...
        text.inputEl.onblur = (e: FocusEvent) => {
          ...
          try { 
            ...
            this.plugin.settings.TaskCompletionStartDate = format(parsedDate, 'yyyy-MM-dd')
            this.plugin.saveData(this.plugin.settings);
          }
          catch {
            alert(`Please specify a date formatted as 'yyyy-MM-dd' in Task Completion Goal Start Date.`)
